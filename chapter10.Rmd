...
date: "2021-10-26"
slug: Rethinking Chapter 10
categories: []
tags: [Rethinking, Bayes, Statistics]
subtitle: ''
summary: 'Common generalized linear models'
...
This is the 11th part of a series where I work through the practice questions of the second edition of Richard McElreaths [Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/).\
Each post covers a new chapter and you can see the posts on previous chapters [here](https://gregor-mathes.netlify.app/tags/rethinking/). This chapter introduces the most common generalized linear models. Note that I skipped chapter 10, as there were no exercises for this chapter. However most of the exercises of this chapter build on chapter 10.  
...
tibble(colours = c(blue, red, grey, yellow), 
       colourname = c("blue", "red", "grey", "yellow")) %>% 
  arrange(colours) %>% 
...
A quick look into chapter 10.2 shows that parameters gives us the equation for the odds, which is just the probability the event happens divided by the probability it does not happen. The log-odds is simply the log of this ratio. $$logodds(p_i) = log \frac{p_i}{1-p_i}$$  
If we plug this in R:
...
We simply has to solve for $p_i$ in the above equation: $$p_i = \frac{exp(logodds(p_i))}{1 + exp(logodds(p_i))}$$  
  
```{r}
exp(3.2)/(1 + exp(3.2))
```  
The `inv_logit` function that comes with the `rethinking`package does exactly the same: 
...
The parameter is given on the log-odds scale, so we need to take the exponential of it to get the odds. 
```{r}
exp(1.7)  
```  
  
An increase of one unit of the predictor variable results in an 5 fold increase in the odds of the outcome. We can easily transform the log-odds to the odds to probability this way. Assuming that the intercept for the given logistic regression is 2 (or any other constant):  

```{r}
tibble(predictor = seq(-1, 1, by = 0.1)) %>% 
  mutate(log_odds = 2 + predictor*1.7, 
         odds = exp(log_odds),
         probability = inv_logit(log_odds)) %>% 
  pivot_longer(cols = log_odds:probability, 
               names_to = "key", 
               values_to = "outcome") %>% 
  ...
  theme_minimal() +
  theme(
    panel.grid.major = element_line(size = 0.65, colour = grey), 
...
An offset is needed when the exposure varies across cases *i*. We used the example with two monasteries in the chapter, where one monastery reported finished books per day and the other one per weeks. To account for this difference in rates, or exposure, we added an offset to the poisson regression:  

$$y_i = Poisson(\mu_i) \\
log(\mu_i) = log(\tau_i) + \alpha + \beta x_i$$
  
This is often called the **Negative Binomial** or **Gamma-Poisson**.  
