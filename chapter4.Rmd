...
But does this actually change the way our model works? We can test this by comparing the posterior predictions:  
  
```{r question 4M7 part 5}
# define weight range
...
# calculate means
reg_line <- link(m4.3, data = data.frame(weight = weight_seq)) %>%
  as_tibble() %>%
  summarise_all(mean) %>%
  pivot_longer(cols = everything()) %>% 
  add_column(weight = weight_seq)
    
tidy_mean <- function(my_model, x_var, x_seq){
  link(my_model, data = data.frame(x_var = x_seq)) %>%
  as_tibble() %>%
  summarise_all(mean) %>%
  pivot_longer(cols = everything()) %>% 
  add_column(x_var = x_seq)
}

tidy_mean(m4.3, x_var = "weight", x_seq = weight_seq)
